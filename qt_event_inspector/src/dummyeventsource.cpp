...
void DummyEventSource::start(int intervalMs) { m_timer.start(intervalMs); }

void DummyEventSource::stop() { m_timer.stop(); }

void DummyEventSource::onTick() {
  ++m_counter;

  static const QStringList names = {
      "PlayerJoined", "ChatMessage", "InventoryUpdate",
      "PositionSync", "Ping",        "CustomEvent"};

  const QString name =
      names.at(QRandomGenerator::global()->bounded(names.size()));
  const QString dir =
      (QRandomGenerator::global()->bounded(100) < 75) ? "IN" : "OUT";
