...
PACKET_HASH_TO_NAME: dict[int, str] = {RAGEHash(name): name for name in PACKET_NAMES}
NAME_TO_PACKET_HASH: dict[str, int] = {name: RAGEHash(name) for name in PACKET_NAMES}

def EventNameToHash(name):
    return NAME_TO_PACKET_HASH.get(name, None)

def HashToEventName(hash):
    return PACKET_HASH_TO_NAME.get(hash, f"{hash:08X}")